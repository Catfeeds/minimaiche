...
class ProductController extends PublicController{
	...
	public function add(){	
		...
		try{
			...
			$array=array(
				'name'=>$_POST['name'] ,
				'intro'=>$_POST['intro']?$_POST['intro']:'含购置税 送一年保险  一年后可买可退' ,
				...
				'gong'=>$_POST['gong'],
				'price'=>$_POST['price'],
				'price_yh'=>$_POST['price_yh'],
				'price_yj'=>$_POST['price_yj'],
				'param' =>$_POST['param'], 
		...
		$pro_allinfo2= $id>0 ? M('product')->where('id='.intval($id))->select() : array();
		//商场信息
	...
	*/
	public function img_del(){
		...
		$check_info = M('product')->where('id="'.intval($pro_id).'"')->find();
		if (!$check_info) {
