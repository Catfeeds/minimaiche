...
class AppointController extends PublicController{
	...
	*/
	public function index(){
		...
		//分页
		$count   = $this->Appoint->where($condition)->count();// 查询满足要求的总记录数
		...
		$list = $this->Appoint->where($condition)->limit($Page->firstRow.','.$Page->listRows)->select();		
	...
	*/
	public function del(){
		...
		$check_info = $this->Appoint->where('id='.intval($id))->find();
		if (!$check_info) {
			$this->error('参数错误！');
			die();
		}
		...
		//修改对应的显示状态
		$up = $this->Appoint->where('id='.intval($id))->delete();
		if ($up) {
			$this->success('操作成功.','index');
		}else{
			$this->error('操作失败.');
	...
	*/
	public function set_lx(){
		...
		//修改对应的显示状态
		$data=array();
		$data['status'] = $check_info['status'] == '1' ?  0 : 1;
		$up = $this->Appoint->where('id='.intval($id))->save($data);
		if ($up) {
			$this->redirect('index');
		}else{
			$this->error('操作失败.');
